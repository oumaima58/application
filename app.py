...
def export_excel():
    ...
    for p in projets_mois:
        if p.get("etat") != "Bloqué":
            ...
            if realisateur in compte_pondere:
                coeff = COEFFICIENTS_TACHES.get(tache, 1)
                compte_pondere[realisateur] += coeff
    ...
    ax.set_title(f"Répartition des tâches - {mois_courant.strftime('%B %Y').capitalize()}")
...
def graph_taches():
    ...
    for i, val in enumerate(valeurs):
        texte = f"{val:.1f}".replace(".", ",")
        ax.text(i, val + 0.1, texte, ha='center', va='bottom')
    ax.set_xlabel("Réalisateurs")
    ax.set_ylabel("Charge effectué par réalisateur")
    ax.set_title(f"Répartition des tâches par réalisateur - {mois_str if mois_str else 'Tous les mois'}")
